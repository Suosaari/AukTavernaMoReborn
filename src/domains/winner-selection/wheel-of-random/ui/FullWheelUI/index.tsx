...
interface RandomWheelProps<TWheelItem extends WheelItem = WheelItem> {
  ...
  /** Fog event: gray the wheel, veil its labels and hide names in the side list. */
  fog?: boolean;
  /** Extra overlays rendered on top of the wheel area (rage meter, flashes, etc.). */
  wheelOverlay?: ReactNode;
  // callbacks
...
export interface RandomWheelController {
  ...
  /** Runs the full spin flow programmatically (used by the rage-mode fast spins). */
...
const FullWheelUI = <TWheelItem extends WheelItem = WheelItem>({
  ...
  // Set right before a programmatic spin to override the spin duration (fast spins).
  const spinDurationOverrideRef = useRef<number | null>(null);
  ...
  // Always points to the latest spin trigger so programmatic spins use fresh state.
  const requestSpinRef = useRef<(durationOverride?: number) => Promise<void>>(async () => {});
  const eliminateItemRef = useRef<(id: string | number) => Promise<void>>(async () => {});
  ...
  const onSpinClick = useCallback(
    async ({ randomnessSource }: Wheel.Settings) => {
      ...
      const duration = durationOverride ?? (randomSpinEnabled ? random.getInt(min!, max!) : spinTime) ?? 20;
  ...
  // Keep the programmatic spin trigger pointing at the latest onSpinClick.
  requestSpinRef.current = async (durationOverride?: number) => {
    spinDurationOverrideRef.current = durationOverride ?? null;
    await handleSubmit(onSpinClick)();
  };

  // Eliminate a specific item (pistol kill): play the eat animation, then drop it.
  eliminateItemRef.current = async (id: string | number) => {
    try {
      await wheelController.current?.eatAnimation(id);
    } catch {
      // animation is best-effort; still remove the item below
    }
    wheelStrategy.removeItem?.(id);
  };
    ...
    <WheelContextProvider controller={wheelController} changeInitialItems={setItemsFromProps}>
      <form className='wheel-content' onSubmit={handleSubmit(onSpinClick)}>
        ...
        {!content && elements.preview && (
          <ItemsPreview allItems={filteredItems} activeItems={items} format={format} hideNames={fog} />
        )}
          ...
          {({ onOptimalSizeChange }) => (
            <div
              style={{
                ...
                width: '100%',
                height: '100%',
                filter: fog ? 'grayscale(1) brightness(0.78)' : undefined,
              ...
              {fog && (
                <div
                  style={{
                    ...
                    // Dense gray cover hiding the whole wheel (names included).
                    background:
                      'radial-gradient(circle at 50% 45%, rgba(170,174,180,0.97) 0%, rgba(120,124,132,0.98) 100%)',
