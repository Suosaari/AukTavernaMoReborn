...
export default defineConfig(({ mode }) => {
  // Загружаем переменные из .env без требования префикса VITE_,
  // чтобы можно было прочитать FRONTEND_PORT для порта dev-сервера.
  const env = loadEnv(mode ?? 'development', process.cwd(), '');
  const frontendPort = Number(env.FRONTEND_PORT) || 1488;
